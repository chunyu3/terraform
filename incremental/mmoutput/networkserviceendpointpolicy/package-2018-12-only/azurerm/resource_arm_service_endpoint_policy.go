...
func resourceArmServiceEndpointPolicyCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
            }
        }
    ...
    if err != nil {
        return fmt.Errorf("Error creating Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Service Endpoint Policy %q (Resource Group %q) ID", name, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmServiceEndpointPolicyRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    ...
    if serviceEndpointPolicyPropertiesFormat := resp.ServiceEndpointPolicyPropertiesFormat; serviceEndpointPolicyPropertiesFormat != nil {
        ...
        if err := d.Set("subnets", flattenArmServiceEndpointPolicySubnet(serviceEndpointPolicyPropertiesFormat.Subnets)); err != nil {
...
func resourceArmServiceEndpointPolicyUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        return fmt.Errorf("Error updating Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for update of Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
...
func resourceArmServiceEndpointPolicyDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error deleting Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Service Endpoint Policy %q (Resource Group %q): %+v", name, resourceGroup, err)
        }
    }
...
func flattenArmServiceEndpointPolicySubnet(input *[]network.Subnet) []interface{} {
    results := make([]interface{}, 0)
    if input == nil {
        return results
    }
