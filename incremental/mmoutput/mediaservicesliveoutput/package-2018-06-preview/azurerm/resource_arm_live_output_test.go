...
func testCheckAzureRMLiveOutputExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMLiveOutputDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).liveOutputsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_live_output" {
            continue
        }
