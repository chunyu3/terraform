...
func testCheckAzureRMHyperVCollectorExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMHyperVCollectorDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).hyperVCollectorsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_hyper_v_collector" {
            continue
        }
