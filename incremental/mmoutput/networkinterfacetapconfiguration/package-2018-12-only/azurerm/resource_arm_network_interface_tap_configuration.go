...
func resourceArmNetworkInterfaceTapConfigurationCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, networkInterfaceName, name)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q): %+v", name, networkInterfaceName, resourceGroup, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, networkInterfaceName, name, tapConfigurationParameters)
    if err != nil {
        return fmt.Errorf("Error creating Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q): %+v", name, networkInterfaceName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q): %+v", name, networkInterfaceName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, networkInterfaceName, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q): %+v", name, networkInterfaceName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q) ID", name, networkInterfaceName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmNetworkInterfaceTapConfigurationRead(d *schema.ResourceData, meta interface{}) error {
    ...
    name := id.Path["tapConfigurations"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q): %+v", name, networkInterfaceName, resourceGroup, err)
    }
...
func resourceArmNetworkInterfaceTapConfigurationDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, networkInterfaceName, name)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q): %+v", name, networkInterfaceName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Network Interface Tap Configuration %q (Network Interface Name %q / Resource Group %q): %+v", name, networkInterfaceName, resourceGroup, err)
        }
    }
