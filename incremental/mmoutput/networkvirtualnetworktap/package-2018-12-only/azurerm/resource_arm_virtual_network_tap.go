...
func resourceArmVirtualNetworkTapCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Virtual Network Tap %q (Resource Group %q): %+v", name, resourceGroup, err)
            }
        }
    ...
    if err != nil {
        return fmt.Errorf("Error creating Virtual Network Tap %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Virtual Network Tap %q (Resource Group %q): %+v", name, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Virtual Network Tap %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Virtual Network Tap %q (Resource Group %q) ID", name, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmVirtualNetworkTapRead(d *schema.ResourceData, meta interface{}) error {
    ...
    name := id.Path["virtualNetworkTaps"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Virtual Network Tap %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
...
func resourceArmVirtualNetworkTapDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error deleting Virtual Network Tap %q (Resource Group %q): %+v", name, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Virtual Network Tap %q (Resource Group %q): %+v", name, resourceGroup, err)
        }
    }
