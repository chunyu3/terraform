...
* `enable_ddos_protection` - (Optional) Indicates if DDoS protection is enabled for all the protected resources in a Virtual Network.

* `enable_vm_protection` - (Optional) Indicates if Vm protection is enabled for all the subnets in a Virtual Network.
