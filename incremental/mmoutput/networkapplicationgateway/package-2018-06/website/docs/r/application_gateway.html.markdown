...
* `bounds` - (Required) One `bound` block defined below.


---

The `bound` block supports the following:

* `min` - (Required) Lower bound on number of Application Gateway instances.

* `max` - (Required) Upper bound on number of Application Gateway instances.
