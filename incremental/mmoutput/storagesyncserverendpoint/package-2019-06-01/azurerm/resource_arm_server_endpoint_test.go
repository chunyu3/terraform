...
func testCheckAzureRMServerEndpointExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMServerEndpointDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).serverEndpointsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_server_endpoint" {
            continue
        }
