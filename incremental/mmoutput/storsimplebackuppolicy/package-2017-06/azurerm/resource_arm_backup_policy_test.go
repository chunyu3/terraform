...
func testCheckAzureRMBackupPolicyExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMBackupPolicyDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).backupPoliciesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_backup_policy" {
            continue
        }
