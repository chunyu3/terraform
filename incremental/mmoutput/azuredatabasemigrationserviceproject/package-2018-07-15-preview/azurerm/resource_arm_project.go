        ...
        Schema: map[string]*schema.Schema{
            ...
            "source_platform": {
                ...
                ValidateFunc: validation.StringInSlice([]string{
                    ...
                    string(azuredatabasemigrationservice.MySQL),
                    string(azuredatabasemigrationservice.PostgreSql),
                    string(azuredatabasemigrationservice.MongoDb),
            ...
            "target_platform": {
                ...
                ValidateFunc: validation.StringInSlice([]string{
                    ...
                    string(azuredatabasemigrationservice.SQLMI),
                    string(azuredatabasemigrationservice.AzureDbForMySql),
                    string(azuredatabasemigrationservice.AzureDbForPostgreSql),
                    string(azuredatabasemigrationservice.MongoDb),
                    string(azuredatabasemigrationservice.Unknown),
                }, false),
            },
...
func resourceArmProjectCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, groupName, serviceName, name)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Project %q (Service Name %q / Group Name %q): %+v", name, serviceName, groupName, err)
            }
        }
    ...
    if _, err := client.CreateOrUpdate(ctx, groupName, serviceName, name, parameters); err != nil {
        return fmt.Errorf("Error creating Project %q (Service Name %q / Group Name %q): %+v", name, serviceName, groupName, err)
    }


    resp, err := client.Get(ctx, groupName, serviceName, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Project %q (Service Name %q / Group Name %q): %+v", name, serviceName, groupName, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Project %q (Service Name %q / Group Name %q) ID", name, serviceName, groupName)
    }
    d.SetId(*resp.ID)
...
func resourceArmProjectRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Project %q (Service Name %q / Group Name %q): %+v", name, serviceName, groupName, err)
    }
...
func resourceArmProjectUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Update(ctx, groupName, serviceName, name, parameters); err != nil {
        return fmt.Errorf("Error updating Project %q (Service Name %q / Group Name %q): %+v", name, serviceName, groupName, err)
    }
...
func resourceArmProjectDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Delete(ctx, groupName, serviceName, name); err != nil {
        return fmt.Errorf("Error deleting Project %q (Service Name %q / Group Name %q): %+v", name, serviceName, groupName, err)
    }
