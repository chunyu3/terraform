...
func testCheckAzureRMServerKeyExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMServerKeyDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).serverKeysClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_server_key" {
            continue
        }
