...
func testCheckAzureRMAssetFilterExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMAssetFilterDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).assetFiltersClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_asset_filter" {
            continue
        }
