...
func testCheckAzureRMStreamingPolicyExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMStreamingPolicyDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).streamingPoliciesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_streaming_policy" {
            continue
        }
