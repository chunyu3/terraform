...
* `allow_tracing` - (Optional) Determines whether tracing can be enabled
...
* `owner_id` - (Optional) User identifier path: /users/{userId}

* `primary_key` - (Optional) Primary subscription key.

* `scope` - (Optional) Scope like /products/{productId} or /apis or /apis/{apiId}
