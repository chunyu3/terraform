...
func resourceArmApiCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, name, apiID, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Api %q (Api %q / Resource Group %q): %+v", name, apiID, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Api %q (Api %q / Resource Group %q): %+v", name, apiID, resourceGroup, err)
    }
