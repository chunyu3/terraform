...
func testCheckAzureRMApiTagDescriptionExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMApiTagDescriptionDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).apiTagDescriptionClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_api_tag_description" {
            continue
        }
