...
func testCheckAzureRMPoolExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMPoolDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).poolsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_pool" {
            continue
        }
