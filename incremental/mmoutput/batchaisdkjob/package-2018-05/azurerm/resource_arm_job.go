...
func resourceArmJobCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, workspaceName, experimentName, name)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", name, experimentName, workspaceName, resourceGroup, err)
            }
        }
    ...
    future, err := client.Create(ctx, resourceGroup, workspaceName, experimentName, name, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", name, experimentName, workspaceName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", name, experimentName, workspaceName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, workspaceName, experimentName, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", name, experimentName, workspaceName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q) ID", name, experimentName, workspaceName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmJobRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", name, experimentName, workspaceName, resourceGroup, err)
    }
...
func resourceArmJobDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, workspaceName, experimentName, name)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", name, experimentName, workspaceName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Job %q (Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", name, experimentName, workspaceName, resourceGroup, err)
        }
    }
