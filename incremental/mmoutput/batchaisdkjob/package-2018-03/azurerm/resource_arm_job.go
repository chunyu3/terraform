...
func resourceArmJobCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Job %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
...
func resourceArmJobDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Job %q (Resource Group %q): %+v", name, resourceGroup, err)
        }
    }
