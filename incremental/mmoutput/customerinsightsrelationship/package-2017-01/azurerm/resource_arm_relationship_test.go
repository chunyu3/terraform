...
func testCheckAzureRMRelationshipExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMRelationshipDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).relationshipsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_relationship" {
            continue
        }
