...
func testCheckAzureRMPublicIPAddresseExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMPublicIPAddresseDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).publicIPAddressesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_public_ip_addresse" {
            continue
        }
