...
func testCheckAzureRMApiIssueCommentExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMApiIssueCommentDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).apiIssueCommentClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_api_issue_comment" {
            continue
        }
