...
func testCheckAzureRMTrustedIdProviderExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMTrustedIdProviderDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).trustedIdProvidersClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_trusted_id_provider" {
            continue
        }
