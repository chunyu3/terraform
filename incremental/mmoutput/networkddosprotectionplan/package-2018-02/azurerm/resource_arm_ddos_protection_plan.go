...
func resourceArmDdosProtectionPlanCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Ddos Protection Plan %q (Resource Group %q): %+v", name, resourceGroup, err)
            }
        }
    ...
    if err != nil {
        return fmt.Errorf("Error creating Ddos Protection Plan %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Ddos Protection Plan %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
...
func resourceArmDdosProtectionPlanRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Ddos Protection Plan %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
...
func resourceArmDdosProtectionPlanDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Ddos Protection Plan %q (Resource Group %q): %+v", name, resourceGroup, err)
        }
    }
