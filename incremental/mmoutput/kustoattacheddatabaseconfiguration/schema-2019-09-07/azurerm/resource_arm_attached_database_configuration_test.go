...
func testCheckAzureRMAttachedDatabaseConfigurationExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMAttachedDatabaseConfigurationDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).attachedDatabaseConfigurationsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_attached_database_configuration" {
            continue
        }
