...
func testCheckAzureRMServerExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMServerDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).serversClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_server" {
            continue
        }
