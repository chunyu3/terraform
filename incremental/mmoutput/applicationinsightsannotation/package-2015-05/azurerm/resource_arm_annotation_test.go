...
func testCheckAzureRMAnnotationExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMAnnotationDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).annotationsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_annotation" {
            continue
        }
