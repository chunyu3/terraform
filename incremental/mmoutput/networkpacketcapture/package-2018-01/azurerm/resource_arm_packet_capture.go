...
func resourceArmPacketCaptureCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Packet Capture %q (Network Watcher Name %q / Resource Group %q): %+v", name, networkWatcherName, resourceGroup, err)
            }
        }
    ...
    future, err := client.Create(ctx, resourceGroup, networkWatcherName, name, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Packet Capture %q (Network Watcher Name %q / Resource Group %q): %+v", name, networkWatcherName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Packet Capture %q (Network Watcher Name %q / Resource Group %q): %+v", name, networkWatcherName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, networkWatcherName, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Packet Capture %q (Network Watcher Name %q / Resource Group %q): %+v", name, networkWatcherName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Packet Capture %q (Network Watcher Name %q / Resource Group %q) ID", name, networkWatcherName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmPacketCaptureRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Packet Capture %q (Network Watcher Name %q / Resource Group %q): %+v", name, networkWatcherName, resourceGroup, err)
    }
...
func resourceArmPacketCaptureDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error deleting Packet Capture %q (Network Watcher Name %q / Resource Group %q): %+v", name, networkWatcherName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Packet Capture %q (Network Watcher Name %q / Resource Group %q): %+v", name, networkWatcherName, resourceGroup, err)
        }
    }
