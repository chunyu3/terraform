...
func resourceArmP2sVpnServerConfigurationCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q): %+v", name, virtualWanName, resourceGroup, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, virtualWanName, name, p2svpnServerConfigurationParameters)
    if err != nil {
        return fmt.Errorf("Error creating P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q): %+v", name, virtualWanName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q): %+v", name, virtualWanName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, virtualWanName, name)
    if err != nil {
        return fmt.Errorf("Error retrieving P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q): %+v", name, virtualWanName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q) ID", name, virtualWanName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmP2sVpnServerConfigurationRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q): %+v", name, virtualWanName, resourceGroup, err)
    }
...
func resourceArmP2sVpnServerConfigurationDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error deleting P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q): %+v", name, virtualWanName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting P2s Vpn Server Configuration %q (Virtual Wan Name %q / Resource Group %q): %+v", name, virtualWanName, resourceGroup, err)
        }
    }
