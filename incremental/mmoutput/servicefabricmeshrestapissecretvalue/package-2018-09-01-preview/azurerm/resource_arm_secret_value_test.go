...
func testCheckAzureRMSecretValueExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMSecretValueDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).secretValueClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_secret_value" {
            continue
        }
