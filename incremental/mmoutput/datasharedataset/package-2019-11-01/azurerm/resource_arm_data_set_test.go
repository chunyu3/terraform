...
func testCheckAzureRMDataSetExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMDataSetDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).dataSetsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_data_set" {
            continue
        }
