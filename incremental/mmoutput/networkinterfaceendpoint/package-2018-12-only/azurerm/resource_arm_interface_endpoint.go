...
func resourceArmInterfaceEndpointCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Interface Endpoint %q (Resource Group %q): %+v", name, resourceGroup, err)
            }
        }
    ...
    if err != nil {
        return fmt.Errorf("Error creating Interface Endpoint %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Interface Endpoint %q (Resource Group %q): %+v", name, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Interface Endpoint %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Interface Endpoint %q (Resource Group %q) ID", name, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmInterfaceEndpointRead(d *schema.ResourceData, meta interface{}) error {
    ...
    name := id.Path["interfaceEndpoints"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Interface Endpoint %q (Resource Group %q): %+v", name, resourceGroup, err)
    }
...
func resourceArmInterfaceEndpointDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error deleting Interface Endpoint %q (Resource Group %q): %+v", name, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Interface Endpoint %q (Resource Group %q): %+v", name, resourceGroup, err)
        }
    }
