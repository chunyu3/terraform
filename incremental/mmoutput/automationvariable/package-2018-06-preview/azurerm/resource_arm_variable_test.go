...
func testCheckAzureRMVariableExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMVariableDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).variableClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_variable" {
            continue
        }
