...
func resourceArmExpressRouteConnectionCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, expressRouteGatewayName, name)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q): %+v", name, expressRouteGatewayName, resourceGroup, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, expressRouteGatewayName, name, putExpressRouteConnectionParameters)
    if err != nil {
        return fmt.Errorf("Error creating Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q): %+v", name, expressRouteGatewayName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q): %+v", name, expressRouteGatewayName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, expressRouteGatewayName, name)
    if err != nil {
        return fmt.Errorf("Error retrieving Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q): %+v", name, expressRouteGatewayName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q) ID", name, expressRouteGatewayName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmExpressRouteConnectionRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q): %+v", name, expressRouteGatewayName, resourceGroup, err)
    }
...
func resourceArmExpressRouteConnectionDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, expressRouteGatewayName, name)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q): %+v", name, expressRouteGatewayName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q): %+v", name, expressRouteGatewayName, resourceGroup, err)
        }
    }
