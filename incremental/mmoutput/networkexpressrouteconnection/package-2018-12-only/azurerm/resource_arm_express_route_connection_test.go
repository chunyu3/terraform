...
func testCheckAzureRMExpressRouteConnectionExists(resourceName string) resource.TestCheckFunc {
    return func(s *terraform.State) error {
        ...
        if resp, err := client.Get(ctx, resourceGroup, expressRouteGatewayName, name); err != nil {
            if utils.ResponseWasNotFound(resp.Response) {
                return fmt.Errorf("Bad: Express Route Connection %q (Express Route Gateway Name %q / Resource Group %q) does not exist", name, expressRouteGatewayName, resourceGroup)
            }
