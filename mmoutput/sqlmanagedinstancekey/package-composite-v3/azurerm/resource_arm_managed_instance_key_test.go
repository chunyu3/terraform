...
func testCheckAzureRMManagedInstanceKeyExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMManagedInstanceKeyDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).managedInstanceKeysClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_managed_instance_key" {
            continue
        }
