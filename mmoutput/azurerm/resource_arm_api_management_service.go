...
func resourceArmApiManagementServiceCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if !utils.ResponseWasNotFound(resp.Response) {
            return tf.ImportAsExistsError("azurerm_api_management_service", *resp.ID)
        }
    }
    ...
    skuCapacity := d.Get("sku_capacity").(int)
    ...
    parameters := apimanagement.ServiceResource{
        ...
        Sku: &apimanagement.ServiceSkuProperties{
            skuCapacity: utils.Int32(int32(skuCapacity)),
            skuName: apimanagement.SkuType(skuName),
        },
        Tags: tags.Expand(t),
    }
...
func resourceArmApiManagementServiceRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if serviceProperties := resp.ServiceProperties; serviceProperties != nil {
        ...
        d.Set("enable_client_certificate", serviceProperties.EnableClientCertificate)
    ...
    if sku := resp.Sku; sku != nil {
        d.Set("sku_capacity", int(*sku.skuCapacity))
        d.Set("sku_name", string(sku.skuName))
    }
