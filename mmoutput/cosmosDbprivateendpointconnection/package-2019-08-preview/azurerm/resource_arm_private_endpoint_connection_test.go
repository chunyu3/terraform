...
func testCheckAzureRMPrivateEndpointConnectionExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMPrivateEndpointConnectionDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).privateEndpointConnectionsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_private_endpoint_connection" {
            continue
        }
