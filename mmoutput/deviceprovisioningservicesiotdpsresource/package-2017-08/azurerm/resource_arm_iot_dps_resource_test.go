...
func testCheckAzureRMIotDpsResourceExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMIotDpsResourceDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).iotDpsResourceClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_iot_dps_resource" {
            continue
        }
