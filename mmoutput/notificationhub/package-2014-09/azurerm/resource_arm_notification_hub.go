...
func expandArmNotificationHubSharedAccessAuthorizationRuleProperties(input []interface{}) *[]notificationhubs.SharedAccessAuthorizationRuleProperties {
    ...
    for _, item := range input {
        ...
        claimType := v["claim_type"].(string)
        claimValue := v["claim_value"].(string)
        rights := v["rights"].([]interface{})
        createdTime := v["created_time"].(string)
        modifiedTime := v["modified_time"].(string)
        revision := v["revision"].(int)

        result := notificationhubs.SharedAccessAuthorizationRuleProperties{
            ClaimType: utils.String(claimType),
            ClaimValue: utils.String(claimValue),
            CreatedTime: convertStringToDate(createdTime),
            KeyName: utils.String(keyName),
            ModifiedTime: convertStringToDate(modifiedTime),
            PrimaryKey: utils.String(primaryKey),
            Revision: utils.Int32(int32(revision)),
...
func flattenArmNotificationHubSharedAccessAuthorizationRuleProperties(input *[]notificationhubs.SharedAccessAuthorizationRuleProperties) []interface{} {
    ...
    for _, item := range *input {
        ...
        if claimType := item.ClaimType; claimType != nil {
            v["claim_type"] = *claimType
        }
        if claimValue := item.ClaimValue; claimValue != nil {
            v["claim_value"] = *claimValue
        }
        if createdTime := item.CreatedTime; createdTime != nil {
            v["created_time"] = (*createdTime).String()
        }
        if keyName := item.KeyName; keyName != nil {
            v["key_name"] = *keyName
        }
        if modifiedTime := item.ModifiedTime; modifiedTime != nil {
            v["modified_time"] = (*modifiedTime).String()
        }
        if primaryKey := item.PrimaryKey; primaryKey != nil {
            v["primary_key"] = *primaryKey
        }
        if revision := item.Revision; revision != nil {
            v["revision"] = int(*revision)
        }
