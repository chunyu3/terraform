...
func testCheckAzureRMSecurityContactExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMSecurityContactDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).securityContactsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_security_contact" {
            continue
        }
