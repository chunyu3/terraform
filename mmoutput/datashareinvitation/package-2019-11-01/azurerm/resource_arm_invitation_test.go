...
func testCheckAzureRMInvitationExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMInvitationDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).invitationsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_invitation" {
            continue
        }
