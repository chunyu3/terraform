...
func testCheckAzureRMSolutionExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMSolutionDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).solutionsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_solution" {
            continue
        }
