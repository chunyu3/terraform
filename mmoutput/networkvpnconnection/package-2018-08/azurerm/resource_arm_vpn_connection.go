...
func resourceArmVpnConnectionCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, gatewayName, connectionName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q): %+v", connectionName, gatewayName, resourceGroup, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, gatewayName, connectionName, vpnConnectionParameters)
    if err != nil {
        return fmt.Errorf("Error creating Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q): %+v", connectionName, gatewayName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q): %+v", connectionName, gatewayName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, gatewayName, connectionName)
    if err != nil {
        return fmt.Errorf("Error retrieving Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q): %+v", connectionName, gatewayName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q) ID", connectionName, gatewayName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmVpnConnectionRead(d *schema.ResourceData, meta interface{}) error {
    ...
    connectionName := id.Path["vpnConnections"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q): %+v", connectionName, gatewayName, resourceGroup, err)
    }
...
func resourceArmVpnConnectionDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, gatewayName, connectionName)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q): %+v", connectionName, gatewayName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Vpn Connection (Connection Name %q / Gateway Name %q / Resource Group %q): %+v", connectionName, gatewayName, resourceGroup, err)
        }
    }
