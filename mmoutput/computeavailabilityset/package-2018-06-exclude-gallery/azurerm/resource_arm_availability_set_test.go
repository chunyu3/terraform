...
func testCheckAzureRMAvailabilitySetExists(resourceName string) resource.TestCheckFunc {
    return func(s *terraform.State) error {
        ...
        if !ok {
            return fmt.Errorf("Availability Set not found: %s", resourceName)
        }

        name := rs.Primary.Attributes["name"]
        resourceGroup := rs.Primary.Attributes["resource_group"]

        client := testAccProvider.Meta().(*ArmClient).availabilitySetsClient
        ...
        if resp, err := client.Get(ctx, resourceGroup, name); err != nil {
            ...
            return fmt.Errorf("Bad: Get on availabilitySetsClient: %+v", err)
        }
