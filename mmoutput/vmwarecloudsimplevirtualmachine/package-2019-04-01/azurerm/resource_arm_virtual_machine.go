...
func resourceArmVirtualMachineCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, virtualMachineName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Virtual Machine (Virtual Machine Name %q / Resource Group %q): %+v", virtualMachineName, resourceGroup, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, virtualMachineName, virtualMachineRequest, referer)
    if err != nil {
        return fmt.Errorf("Error creating Virtual Machine (Virtual Machine Name %q / Referer %q / Resource Group %q): %+v", virtualMachineName, referer, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Virtual Machine (Virtual Machine Name %q / Referer %q / Resource Group %q): %+v", virtualMachineName, referer, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, virtualMachineName)
    if err != nil {
        return fmt.Errorf("Error retrieving Virtual Machine (Virtual Machine Name %q / Resource Group %q): %+v", virtualMachineName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Virtual Machine (Virtual Machine Name %q / Resource Group %q) ID", virtualMachineName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmVirtualMachineRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Virtual Machine (Virtual Machine Name %q / Resource Group %q): %+v", virtualMachineName, resourceGroup, err)
    }
...
func resourceArmVirtualMachineUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Update(ctx, resourceGroup, virtualMachineName, virtualMachineRequest)
    if err != nil {
        return fmt.Errorf("Error updating Virtual Machine (Virtual Machine Name %q / Resource Group %q): %+v", virtualMachineName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for update of Virtual Machine (Virtual Machine Name %q / Resource Group %q): %+v", virtualMachineName, resourceGroup, err)
    }
...
func resourceArmVirtualMachineDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, virtualMachineName, referer)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Virtual Machine (Virtual Machine Name %q / Referer %q / Resource Group %q): %+v", virtualMachineName, referer, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Virtual Machine (Virtual Machine Name %q / Referer %q / Resource Group %q): %+v", virtualMachineName, referer, resourceGroup, err)
        }
    }
