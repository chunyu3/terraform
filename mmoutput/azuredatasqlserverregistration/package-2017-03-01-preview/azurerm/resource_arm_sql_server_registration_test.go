...
func testCheckAzureRMSqlServerRegistrationExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMSqlServerRegistrationDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).sqlServerRegistrationsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_sql_server_registration" {
            continue
        }
