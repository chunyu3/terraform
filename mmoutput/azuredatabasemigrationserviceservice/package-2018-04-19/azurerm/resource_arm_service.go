...
func resourceArmServiceCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, groupName, serviceName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, groupName, serviceName, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
    }


    resp, err := client.Get(ctx, groupName, serviceName)
    if err != nil {
        return fmt.Errorf("Error retrieving Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Service (Service Name %q / Group Name %q) ID", serviceName, groupName)
    }
    d.SetId(*resp.ID)
...
func resourceArmServiceRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
    }
...
func resourceArmServiceUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Update(ctx, groupName, serviceName, parameters)
    if err != nil {
        return fmt.Errorf("Error updating Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for update of Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
    }
...
func resourceArmServiceDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, groupName, serviceName)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Service (Service Name %q / Group Name %q): %+v", serviceName, groupName, err)
        }
    }
