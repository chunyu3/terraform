...
func resourceArmRedisLinkedServerCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Redis Linked Server %q (Linked Server Name %q / Resource Group %q): %+v", name, linkedServerName, resourceGroup, err)
            }
        }
    ...
    if err != nil {
        return fmt.Errorf("Error creating Redis Linked Server %q (Linked Server Name %q / Resource Group %q): %+v", name, linkedServerName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Redis Linked Server %q (Linked Server Name %q / Resource Group %q): %+v", name, linkedServerName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, name, linkedServerName)
    if err != nil {
        return fmt.Errorf("Error retrieving Redis Linked Server %q (Linked Server Name %q / Resource Group %q): %+v", name, linkedServerName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Redis Linked Server %q (Linked Server Name %q / Resource Group %q) ID", name, linkedServerName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmRedisLinkedServerRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Redis Linked Server %q (Linked Server Name %q / Resource Group %q): %+v", name, linkedServerName, resourceGroup, err)
    }
...
func resourceArmRedisLinkedServerDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Delete(ctx, resourceGroup, name, linkedServerName); err != nil {
        return fmt.Errorf("Error deleting Redis Linked Server %q (Linked Server Name %q / Resource Group %q): %+v", name, linkedServerName, resourceGroup, err)
    }
