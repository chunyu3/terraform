...
func testCheckAzureRMApplianceDefinitionExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMApplianceDefinitionDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).applianceDefinitionsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_appliance_definition" {
            continue
        }
