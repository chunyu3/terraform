...
func resourceArmCloudConnectorCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, connectorName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Cloud Connector (Connector Name %q): %+v", connectorName, err)
            }
        }
    ...
    if _, err := client.CreateOrUpdate(ctx, connectorName, connector); err != nil {
        return fmt.Errorf("Error creating Cloud Connector (Connector Name %q): %+v", connectorName, err)
    }


    resp, err := client.Get(ctx, connectorName)
    if err != nil {
        return fmt.Errorf("Error retrieving Cloud Connector (Connector Name %q): %+v", connectorName, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Cloud Connector (Connector Name %q) ID", connectorName)
    }
    d.SetId(*resp.ID)
...
func resourceArmCloudConnectorRead(d *schema.ResourceData, meta interface{}) error {
    ...
    connectorName := id.Path["cloudConnectors"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Cloud Connector (Connector Name %q): %+v", connectorName, err)
    }
...
func resourceArmCloudConnectorUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Update(ctx, connectorName, connector); err != nil {
        return fmt.Errorf("Error updating Cloud Connector (Connector Name %q): %+v", connectorName, err)
    }
...
func resourceArmCloudConnectorDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Delete(ctx, connectorName); err != nil {
        return fmt.Errorf("Error deleting Cloud Connector (Connector Name %q): %+v", connectorName, err)
    }
