...
func resourceArmReplicationProtectionContainerMappingCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceName, resourceGroup, fabricName, protectionContainerName, mappingName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q): %+v", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName, err)
            }
        }
    ...
    future, err := client.Create(ctx, resourceName, resourceGroup, fabricName, protectionContainerName, mappingName, creationInput)
    if err != nil {
        return fmt.Errorf("Error creating Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q): %+v", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q): %+v", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName, err)
    }


    resp, err := client.Get(ctx, resourceName, resourceGroup, fabricName, protectionContainerName, mappingName)
    if err != nil {
        return fmt.Errorf("Error retrieving Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q): %+v", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q) ID", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName)
    }
    d.SetId(*resp.ID)
...
func resourceArmReplicationProtectionContainerMappingRead(d *schema.ResourceData, meta interface{}) error {
    ...
    mappingName := id.Path["replicationProtectionContainerMappings"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q): %+v", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName, err)
    }
...
func resourceArmReplicationProtectionContainerMappingDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceName, resourceGroup, fabricName, protectionContainerName, mappingName)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q): %+v", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Replication Protection Container Mapping (Mapping Name %q / Protection Container Name %q / Fabric Name %q / Resource Group %q / Resource Name %q): %+v", mappingName, protectionContainerName, fabricName, resourceGroup, resourceName, err)
        }
    }
