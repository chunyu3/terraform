...
func testCheckAzureRMJobCredentialExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMJobCredentialDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).jobCredentialsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_job_credential" {
            continue
        }
