...
func testCheckAzureRMDiagnosticExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMDiagnosticDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).diagnosticClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_diagnostic" {
            continue
        }
