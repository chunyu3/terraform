...
func testCheckAzureRMViewExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMViewDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).viewsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_view" {
            continue
        }
