...
func testCheckAzureRMRemoteRenderingAccountExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMRemoteRenderingAccountDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).remoteRenderingAccountsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_remote_rendering_account" {
            continue
        }
