...
func resourceArmSnapshotCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    snapshot := compute.SnapshotUpdate{
        ...
        SnapshotUpdateProperties: &compute.SnapshotUpdateProperties{
...
func resourceArmSnapshotRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if snapshotUpdateProperties := resp.SnapshotUpdateProperties; snapshotUpdateProperties != nil {
        if err := d.Set("creation_data", flattenArmSnapshotCreationData(snapshotUpdateProperties.CreationData)); err != nil {
            return fmt.Errorf("Error setting `creation_data`: %+v", err)
        }
        d.Set("disk_size_bytes", int(*snapshotUpdateProperties.DiskSizeBytes))
        d.Set("disk_size_gb", int(*snapshotUpdateProperties.DiskSizeGB))
        if err := d.Set("encryption", flattenArmSnapshotEncryption(snapshotUpdateProperties.Encryption)); err != nil {
            return fmt.Errorf("Error setting `encryption`: %+v", err)
        }
        if err := d.Set("encryption_settings_collection", flattenArmSnapshotEncryptionSettingsCollection(snapshotUpdateProperties.EncryptionSettingsCollection)); err != nil {
            return fmt.Errorf("Error setting `encryption_settings_collection`: %+v", err)
        }
        d.Set("hyper_vgeneration", string(snapshotUpdateProperties.HyperVGeneration))
        d.Set("incremental", snapshotUpdateProperties.Incremental)
        d.Set("os_type", string(snapshotUpdateProperties.OsType))
        d.Set("provisioning_state", snapshotUpdateProperties.ProvisioningState)
        d.Set("time_created", (snapshotUpdateProperties.TimeCreated).String())
        d.Set("unique_id", snapshotUpdateProperties.UniqueID)
    }
...
func flattenArmSnapshotEncryption(input *compute.Encryption) []interface{} {
    if input == nil {
        return make([]interface{}, 0)
    }
