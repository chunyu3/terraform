                    ...
                    Schema: map[string]*schema.Schema{
                        "type": {
                            ...
                            ValidateFunc: validation.StringInSlice([]string{
                                ...
                                string(web.SystemAssigned, UserAssigned),
                                string(web.None),
                            }, false),
...
func resourceArmWebAppCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    geoDistributions := d.Get("geo_distributions").([]interface{})
    ...
    siteEnvelope := web.Site{
        ...
        Site_properties: &web.Site_properties{
            ...
            GeoDistributions: expandArmWebAppGeoDistribution(geoDistributions),
...
func resourceArmWebAppRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if siteProperties := resp.Site_properties; siteProperties != nil {
        ...
        if err := d.Set("geo_distributions", flattenArmWebAppGeoDistribution(siteProperties.GeoDistributions)); err != nil {
            return fmt.Errorf("Error setting `geo_distributions`: %+v", err)
        }
...
func expandArmWebAppGeoDistribution(input []interface{}) *[]web.GeoDistribution {
    results := make([]web.GeoDistribution, 0)
    for _, item := range input {
        v := item.(map[string]interface{})
        location := azure.NormalizeLocation(v["location"].(string))
        numberOfWorkers := v["number_of_workers"].(int)

        result := web.GeoDistribution{
            Location: utils.String(location),
...
func expandArmWebAppSiteConfig(input []interface{}) *web.SiteConfig {
    ...
    azureStorageAccounts := v["azure_storage_accounts"].(map[string]interface{})
    ...
    reservedInstanceCount := v["reserved_instance_count"].(int)
    ...
    result := web.SiteConfig{
        ...
        AzureStorageAccounts: utils.ExpandKeyValuePairs(azureStorageAccounts),
        ...
        ReservedInstanceCount: utils.Int32(int32(reservedInstanceCount)),
...
func flattenArmWebAppGeoDistribution(input *[]web.GeoDistribution) []interface{} {
    ...
    for _, item := range *input {
        ...
        if numberOfWorkers := item.NumberOfWorkers; numberOfWorkers != nil {
            v["number_of_workers"] = int(*numberOfWorkers)
        }
...
func flattenArmWebAppSiteConfig(input *web.SiteConfig) []interface{} {
    ...
    result["azure_storage_accounts"] = utils.FlattenKeyValuePairs(input.AzureStorageAccounts)
    ...
    if reservedInstanceCount := input.ReservedInstanceCount; reservedInstanceCount != nil {
        result["reserved_instance_count"] = int(*reservedInstanceCount)
    }
