...
func resourceArmVpnGatewayCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Vpn Gateway (Gateway Name %q / Resource Group %q): %+v", gatewayName, resourceGroup, err)
            }
        }
    ...
    vpnGatewayParameters := network.VpnGateway{
        ...
        VpnGatewayProperties: &network.VpnGatewayProperties{
            ...
            Policies: expandArmVpnGatewayPolicies(policies),
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, gatewayName, vpnGatewayParameters)
    if err != nil {
        return fmt.Errorf("Error creating Vpn Gateway (Gateway Name %q / Resource Group %q): %+v", gatewayName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Vpn Gateway (Gateway Name %q / Resource Group %q): %+v", gatewayName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, gatewayName)
    if err != nil {
        return fmt.Errorf("Error retrieving Vpn Gateway (Gateway Name %q / Resource Group %q): %+v", gatewayName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Vpn Gateway (Gateway Name %q / Resource Group %q) ID", gatewayName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmVpnGatewayRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Vpn Gateway (Gateway Name %q / Resource Group %q): %+v", gatewayName, resourceGroup, err)
    }
    ...
    if vpnGatewayProperties := resp.VpnGatewayProperties; vpnGatewayProperties != nil {
        ...
        if err := d.Set("policies", flattenArmVpnGatewayPolicies(vpnGatewayProperties.Policies)); err != nil {
...
func resourceArmVpnGatewayDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error deleting Vpn Gateway (Gateway Name %q / Resource Group %q): %+v", gatewayName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Vpn Gateway (Gateway Name %q / Resource Group %q): %+v", gatewayName, resourceGroup, err)
        }
    }
...
func expandArmVpnGatewayPolicies(input []interface{}) *network.Policies {
    if len(input) == 0 {
        return nil
    }
    v := input[0].(map[string]interface{})

    allowBranchToBranchTraffic := v["allow_branch_to_branch_traffic"].(bool)
    allowVnetToVnetTraffic := v["allow_vnet_to_vnet_traffic"].(bool)

    result := network.Policies{
...
func flattenArmVpnGatewayPolicies(input *network.Policies) []interface{} {
    if input == nil {
        return make([]interface{}, 0)
    }
    ...
    if allowBranchToBranchTraffic := input.AllowBranchToBranchTraffic; allowBranchToBranchTraffic != nil {
        result["allow_branch_to_branch_traffic"] = *allowBranchToBranchTraffic
    }
    if allowVnetToVnetTraffic := input.AllowVnetToVnetTraffic; allowVnetToVnetTraffic != nil {
        result["allow_vnet_to_vnet_traffic"] = *allowVnetToVnetTraffic
    }
