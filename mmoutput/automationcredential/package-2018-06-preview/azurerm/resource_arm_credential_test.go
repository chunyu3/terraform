...
func testCheckAzureRMCredentialExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMCredentialDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).credentialClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_credential" {
            continue
        }
