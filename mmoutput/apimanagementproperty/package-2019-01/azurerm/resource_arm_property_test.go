...
func testCheckAzureRMPropertyExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMPropertyDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).propertyClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_property" {
            continue
        }
