...
func resourceArmApplicationGatewayCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    firewallPolicy := d.Get("firewall_policy").([]interface{})
    ...
    parameters := network.ApplicationGateway{
        ...
        ApplicationGatewayPropertiesFormat: &network.ApplicationGatewayPropertiesFormat{
            ...
            FirewallPolicy: expandArmApplicationGatewaySubResource(firewallPolicy),
...
func resourceArmApplicationGatewayRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if applicationGatewayPropertiesFormat := resp.ApplicationGatewayPropertiesFormat; applicationGatewayPropertiesFormat != nil {
        ...
        if err := d.Set("firewall_policy", flattenArmApplicationGatewaySubResource(applicationGatewayPropertiesFormat.FirewallPolicy)); err != nil {
            return fmt.Errorf("Error setting `firewall_policy`: %+v", err)
        }
...
func expandArmApplicationGatewayApplicationGatewayAutoscaleConfiguration(input []interface{}) *network.ApplicationGatewayAutoscaleConfiguration {
    ...
    maxCapacity := v["max_capacity"].(int)

    result := network.ApplicationGatewayAutoscaleConfiguration{
        MaxCapacity: utils.Int32(int32(maxCapacity)),
...
func expandArmApplicationGatewayApplicationGatewayRewriteRule(input []interface{}) *[]network.ApplicationGatewayRewriteRule {
    ...
    for _, item := range input {
        v := item.(map[string]interface{})
        name := v["name"].(string)
        ruleSequence := v["rule_sequence"].(int)

        result := network.ApplicationGatewayRewriteRule{
            Name: utils.String(name),
            RuleSequence: utils.Int(ruleSequence),
        }
...
func flattenArmApplicationGatewayApplicationGatewayAutoscaleConfiguration(input *network.ApplicationGatewayAutoscaleConfiguration) []interface{} {
    ...
    if maxCapacity := input.MaxCapacity; maxCapacity != nil {
        result["max_capacity"] = int(*maxCapacity)
    }
...
func flattenArmApplicationGatewayApplicationGatewayRewriteRule(input *[]network.ApplicationGatewayRewriteRule) []interface{} {
    ...
    for _, item := range *input {
        ...
        if ruleSequence := item.RuleSequence; ruleSequence != nil {
            v["rule_sequence"] = *ruleSequence
        }
