...
func testCheckAzureRMAccountFilterExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMAccountFilterDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).accountFiltersClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_account_filter" {
            continue
        }
