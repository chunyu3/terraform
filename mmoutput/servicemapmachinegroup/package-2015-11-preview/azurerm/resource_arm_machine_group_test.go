...
func testCheckAzureRMMachineGroupExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMMachineGroupDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).machineGroupsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_machine_group" {
            continue
        }
