...
* `storage_account_type` - (Optional) Specifies the storage account type for the managed disk. UltraSSD_LRS cannot be used with OS Disk. Defaults to `Standard_LRS`.
