...
func resourceArmSubscriptionCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.CreateOrUpdate(ctx, resourceGroup, serviceName, sid, notify, parameters); err != nil {
        return fmt.Errorf("Error creating Subscription (Notify %q / Sid %q / Service Name %q / Resource Group %q): %+v", notify, sid, serviceName, resourceGroup, err)
    }
...
func resourceArmSubscriptionUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Update(ctx, resourceGroup, serviceName, sid, notify, parameters); err != nil {
        return fmt.Errorf("Error updating Subscription (Notify %q / Sid %q / Service Name %q / Resource Group %q): %+v", notify, sid, serviceName, resourceGroup, err)
    }
