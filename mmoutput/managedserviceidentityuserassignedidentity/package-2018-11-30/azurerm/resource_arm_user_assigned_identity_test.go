...
func testCheckAzureRMUserAssignedIdentityExists(resourceName string) resource.TestCheckFunc {
    return func(s *terraform.State) error {
        ...
        if !ok {
            return fmt.Errorf("User Assigned Identity not found: %s", resourceName)
        }
        ...
        if resp, err := client.Get(ctx, resourceGroup, resourceName); err != nil {
            if utils.ResponseWasNotFound(resp.Response) {
                return fmt.Errorf("Bad: User Assigned Identity (Resource Name %q / Resource Group %q) does not exist", resourceName, resourceGroup)
            }
...
func testCheckAzureRMUserAssignedIdentityDestroy(s *terraform.State) error {
    ...
    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_user_assigned_identity" {
            continue
        }
