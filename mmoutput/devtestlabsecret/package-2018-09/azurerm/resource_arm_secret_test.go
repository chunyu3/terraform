...
func testCheckAzureRMSecretExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMSecretDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).secretsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_secret" {
            continue
        }
