...
func resourceArmRedisFirewallRuleCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        ...
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Redis Firewall Rule (Rule Name %q / Cache Name %q / Resource Group %q): %+v", ruleName, cacheName, resourceGroup, err)
            }
        }
    ...
    if _, err := client.CreateOrUpdate(ctx, resourceGroup, cacheName, ruleName, parameters); err != nil {
        return fmt.Errorf("Error creating Redis Firewall Rule (Rule Name %q / Cache Name %q / Resource Group %q): %+v", ruleName, cacheName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, cacheName, ruleName)
    if err != nil {
        return fmt.Errorf("Error retrieving Redis Firewall Rule (Rule Name %q / Cache Name %q / Resource Group %q): %+v", ruleName, cacheName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Redis Firewall Rule (Rule Name %q / Cache Name %q / Resource Group %q) ID", ruleName, cacheName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmRedisFirewallRuleRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Redis Firewall Rule (Rule Name %q / Cache Name %q / Resource Group %q): %+v", ruleName, cacheName, resourceGroup, err)
    }
...
func resourceArmRedisFirewallRuleDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Delete(ctx, resourceGroup, cacheName, ruleName); err != nil {
        return fmt.Errorf("Error deleting Redis Firewall Rule (Rule Name %q / Cache Name %q / Resource Group %q): %+v", ruleName, cacheName, resourceGroup, err)
    }
