...
func testCheckAzureRMStepExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMStepDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).stepsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_step" {
            continue
        }
