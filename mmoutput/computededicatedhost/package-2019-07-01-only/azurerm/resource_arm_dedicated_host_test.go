...
func testCheckAzureRMDedicatedHostExists(resourceName string) resource.TestCheckFunc {
    return func(s *terraform.State) error {
        ...
        client := testAccProvider.Meta().(*ArmClient).dedicatedHostsClient
        ctx := testAccProvider.Meta().(*ArmClient).StopContext

        if resp, err := client.Get(ctx, resourceGroup, hostGroupName, name); err != nil {
            if utils.ResponseWasNotFound(resp.Response) {
                return fmt.Errorf("Bad: Dedicated Host %q (Host Group Name %q / Resource Group %q) does not exist", name, hostGroupName, resourceGroup)
            }
