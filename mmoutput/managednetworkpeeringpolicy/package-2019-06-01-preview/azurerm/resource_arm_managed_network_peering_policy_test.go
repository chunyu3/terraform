...
func testCheckAzureRMManagedNetworkPeeringPolicyExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMManagedNetworkPeeringPolicyDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).managedNetworkPeeringPoliciesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_managed_network_peering_policy" {
            continue
        }
