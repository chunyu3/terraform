...
func resourceArmDdosCustomPolicyCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    ddosCustomPolicyName := d.Get("ddos_custom_policy_name").(string)

    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, ddosCustomPolicyName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q): %+v", ddosCustomPolicyName, resourceGroup, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, ddosCustomPolicyName, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q): %+v", ddosCustomPolicyName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q): %+v", ddosCustomPolicyName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, ddosCustomPolicyName)
    if err != nil {
        return fmt.Errorf("Error retrieving Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q): %+v", ddosCustomPolicyName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q) ID", ddosCustomPolicyName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmDdosCustomPolicyRead(d *schema.ResourceData, meta interface{}) error {
    ...
    ddosCustomPolicyName := id.Path["ddosCustomPolicies"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q): %+v", ddosCustomPolicyName, resourceGroup, err)
    }
    ...
    d.Set("ddos_custom_policy_name", ddosCustomPolicyName)
...
func resourceArmDdosCustomPolicyDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, ddosCustomPolicyName)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q): %+v", ddosCustomPolicyName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Ddos Custom Policy (Ddos Custom Policy Name %q / Resource Group %q): %+v", ddosCustomPolicyName, resourceGroup, err)
        }
    }
