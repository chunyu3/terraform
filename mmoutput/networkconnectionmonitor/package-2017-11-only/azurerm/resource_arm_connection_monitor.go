...
func resourceArmConnectionMonitorCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    connectionMonitorName := d.Get("connection_monitor_name").(string)
    networkWatcherName := d.Get("network_watcher_name").(string)

    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, networkWatcherName, connectionMonitorName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q): %+v", connectionMonitorName, networkWatcherName, resourceGroup, err)
            }
        }
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, networkWatcherName, connectionMonitorName, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q): %+v", connectionMonitorName, networkWatcherName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q): %+v", connectionMonitorName, networkWatcherName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, networkWatcherName, connectionMonitorName)
    if err != nil {
        return fmt.Errorf("Error retrieving Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q): %+v", connectionMonitorName, networkWatcherName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q) ID", connectionMonitorName, networkWatcherName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmConnectionMonitorRead(d *schema.ResourceData, meta interface{}) error {
    ...
    connectionMonitorName := id.Path["connectionMonitors"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q): %+v", connectionMonitorName, networkWatcherName, resourceGroup, err)
    }
    ...
    d.Set("connection_monitor_name", connectionMonitorName)
...
func resourceArmConnectionMonitorDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, networkWatcherName, connectionMonitorName)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q): %+v", connectionMonitorName, networkWatcherName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Connection Monitor (Connection Monitor Name %q / Network Watcher Name %q / Resource Group %q): %+v", connectionMonitorName, networkWatcherName, resourceGroup, err)
        }
    }
