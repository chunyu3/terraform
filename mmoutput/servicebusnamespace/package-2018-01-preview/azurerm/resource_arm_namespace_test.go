...
func testCheckAzureRMNamespaceExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMNamespaceDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).namespacesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_namespace" {
            continue
        }
