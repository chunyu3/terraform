...
func resourceArmApplicationCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    applicationResourceName := d.Get("application_resource_name").(string)

    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, applicationResourceName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Application (Application Resource Name %q / Resource Group %q): %+v", applicationResourceName, resourceGroup, err)
            }
        }
    ...
    if _, err := client.Create(ctx, resourceGroup, applicationResourceName, applicationResourceDescription); err != nil {
        return fmt.Errorf("Error creating Application (Application Resource Name %q / Resource Group %q): %+v", applicationResourceName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, applicationResourceName)
    if err != nil {
        return fmt.Errorf("Error retrieving Application (Application Resource Name %q / Resource Group %q): %+v", applicationResourceName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Application (Application Resource Name %q / Resource Group %q) ID", applicationResourceName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmApplicationRead(d *schema.ResourceData, meta interface{}) error {
    ...
    applicationResourceName := id.Path["applications"]
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Application (Application Resource Name %q / Resource Group %q): %+v", applicationResourceName, resourceGroup, err)
    }
    ...
    d.Set("application_resource_name", applicationResourceName)
...
func resourceArmApplicationDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Delete(ctx, resourceGroup, applicationResourceName); err != nil {
        return fmt.Errorf("Error deleting Application (Application Resource Name %q / Resource Group %q): %+v", applicationResourceName, resourceGroup, err)
    }
