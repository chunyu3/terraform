...
func resourceArmApplicationCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, applicationName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Application (Application Name %q / Resource Group %q): %+v", applicationName, resourceGroup, err)
            }
        }
    ...
    if _, err := client.Create(ctx, resourceGroup, applicationName, applicationResourceDescription); err != nil {
        return fmt.Errorf("Error creating Application (Application Name %q / Resource Group %q): %+v", applicationName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, applicationName)
    if err != nil {
        return fmt.Errorf("Error retrieving Application (Application Name %q / Resource Group %q): %+v", applicationName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Application (Application Name %q / Resource Group %q) ID", applicationName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmApplicationRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Application (Application Name %q / Resource Group %q): %+v", applicationName, resourceGroup, err)
    }
...
func resourceArmApplicationDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Delete(ctx, resourceGroup, applicationName); err != nil {
        return fmt.Errorf("Error deleting Application (Application Name %q / Resource Group %q): %+v", applicationName, resourceGroup, err)
    }
