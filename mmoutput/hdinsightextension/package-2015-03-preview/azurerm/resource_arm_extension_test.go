...
func testCheckAzureRMExtensionExists(resourceName string) resource.TestCheckFunc {
    return func(s *terraform.State) error {
        ...
        client := testAccProvider.Meta().(*ArmClient).extensionClient
        ...
        if resp, err := client.Get(ctx, resourceGroup, clusterName, extensionName); err != nil {
            ...
            return fmt.Errorf("Bad: Get on extensionClient: %+v", err)
        }
...
func testCheckAzureRMExtensionDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).extensionClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_extension" {
            continue
        }
