...
func testCheckAzureRMAssetExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMAssetDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).assetsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_asset" {
            continue
        }
