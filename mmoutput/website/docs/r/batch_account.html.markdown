...
* `auto_storage_account_id` - (Required) The resource ID of the storage account to be used for auto-storage account.
...
* `auto_storage_account_id` - The UTC time at which storage keys were last synchronized with the Batch account.
