...
func testCheckAzureRMWatcherExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMWatcherDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).watcherClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_watcher" {
            continue
        }
