...
func testCheckAzureRMDedicatedCloudServiceExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMDedicatedCloudServiceDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).dedicatedCloudServicesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_dedicated_cloud_service" {
            continue
        }
