...
func testCheckAzureRMVpnSiteExists(resourceName string) resource.TestCheckFunc {
    return func(s *terraform.State) error {
        ...
        vpnSiteName := rs.Primary.Attributes["vpn_site_name"]

        client := testAccProvider.Meta().(*ArmClient).vpnSitesClient
        ctx := testAccProvider.Meta().(*ArmClient).StopContext

        if resp, err := client.Get(ctx, resourceGroup, vpnSiteName); err != nil {
            if utils.ResponseWasNotFound(resp.Response) {
                return fmt.Errorf("Bad: Vpn Site (Vpn Site Name %q / Resource Group %q) does not exist", vpnSiteName, resourceGroup)
            }
