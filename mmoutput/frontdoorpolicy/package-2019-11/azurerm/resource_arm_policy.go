...
func resourceArmPolicyCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.CreateOrUpdate(ctx, resourceGroup, policyName, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Policy (Policy Name %q / Resource Group %q): %+v", policyName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Policy (Policy Name %q / Resource Group %q): %+v", policyName, resourceGroup, err)
    }
