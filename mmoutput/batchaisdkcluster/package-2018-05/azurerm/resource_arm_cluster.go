...
func resourceArmClusterCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, workspaceName, clusterName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
            }
        }
    ...
    future, err := client.Create(ctx, resourceGroup, workspaceName, clusterName, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, workspaceName, clusterName)
    if err != nil {
        return fmt.Errorf("Error retrieving Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q) ID", clusterName, workspaceName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmClusterRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
    }
...
func resourceArmClusterUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if _, err := client.Update(ctx, resourceGroup, workspaceName, clusterName, parameters); err != nil {
        return fmt.Errorf("Error updating Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
    }
...
func resourceArmClusterDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, workspaceName, clusterName)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Cluster (Cluster Name %q / Workspace Name %q / Resource Group %q): %+v", clusterName, workspaceName, resourceGroup, err)
        }
    }
...
func expandArmClusterNodeSetup(input []interface{}) *batchaisdk.NodeSetup {
    ...
    performanceCountersSettings := v["performance_counters_settings"].([]interface{})

    result := batchaisdk.NodeSetup{
        MountVolumes: expandArmClusterMountVolumes(mountVolumes),
        PerformanceCountersSettings: expandArmClusterPerformanceCountersSettings(performanceCountersSettings),
...
func expandArmClusterPerformanceCountersSettings(input []interface{}) *batchaisdk.PerformanceCountersSettings {
    if len(input) == 0 {
        return nil
    }
    v := input[0].(map[string]interface{})

    appInsightsReference := v["app_insights_reference"].([]interface{})

    result := batchaisdk.PerformanceCountersSettings{
        AppInsightsReference: expandArmClusterAppInsightsReference(appInsightsReference),
    }
    return &result
}
...
func expandArmClusterSetupTask(input []interface{}) *batchaisdk.SetupTask {
    ...
    result := batchaisdk.SetupTask{
        ...
        EnvironmentVariables: expandArmClusterEnvironmentVariable(environmentVariables),
        Secrets: expandArmClusterEnvironmentVariableWithSecretValue(secrets),
...
func expandArmClusterImageReference(input []interface{}) *batchaisdk.ImageReference {
    ...
    virtualMachineImageId := v["virtual_machine_image_id"].(string)
    ...
    result := batchaisdk.ImageReference{
        ...
        VirtualMachineImageID: utils.String(virtualMachineImageId),
    }
    return &result
}
...
func expandArmClusterAppInsightsReference(input []interface{}) *batchaisdk.AppInsightsReference {
    if len(input) == 0 {
        return nil
    }
    v := input[0].(map[string]interface{})

    instrumentationKey := v["instrumentation_key"].(string)

    result := batchaisdk.AppInsightsReference{
        InstrumentationKey: utils.String(instrumentationKey),
    }
    return &result
}

func expandArmClusterEnvironmentVariable(input []interface{}) *[]batchaisdk.EnvironmentVariable {
...
func expandArmClusterEnvironmentVariableWithSecretValue(input []interface{}) *[]batchaisdk.EnvironmentVariableWithSecretValue {
...
func flattenArmClusterNodeSetup(input *batchaisdk.NodeSetup) []interface{} {
    ...
    result["performance_counters_settings"] = flattenArmClusterPerformanceCountersSettings(input.PerformanceCountersSettings)
...
func flattenArmClusterPerformanceCountersSettings(input *batchaisdk.PerformanceCountersSettings) []interface{} {
    if input == nil {
        return make([]interface{}, 0)
    }
    ...
    result["app_insights_reference"] = flattenArmClusterAppInsightsReference(input.AppInsightsReference)
...
func flattenArmClusterSetupTask(input *batchaisdk.SetupTask) []interface{} {
    ...
    result["environment_variables"] = flattenArmClusterEnvironmentVariable(input.EnvironmentVariables)
    result["secrets"] = flattenArmClusterEnvironmentVariableWithSecretValue(input.Secrets)
...
func flattenArmClusterImageReference(input *batchaisdk.ImageReference) []interface{} {
    ...
    if virtualMachineImageId := input.VirtualMachineImageID; virtualMachineImageId != nil {
        result["virtual_machine_image_id"] = *virtualMachineImageId
    }
...
func flattenArmClusterAppInsightsReference(input *batchaisdk.AppInsightsReference) []interface{} {
    if input == nil {
        return make([]interface{}, 0)
    }
    ...
    if instrumentationKey := input.InstrumentationKey; instrumentationKey != nil {
        result["instrumentation_key"] = *instrumentationKey
    }

    return []interface{}{result}
}

func flattenArmClusterEnvironmentVariable(input *[]batchaisdk.EnvironmentVariable) []interface{} {
    results := make([]interface{}, 0)
    if input == nil {
        return results
    }
...
func flattenArmClusterEnvironmentVariableWithSecretValue(input *[]batchaisdk.EnvironmentVariableWithSecretValue) []interface{} {
    results := make([]interface{}, 0)
    if input == nil {
        return results
    }
