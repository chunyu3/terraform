                ...
                Elem: &schema.Resource{
                    Schema: map[string]*schema.Schema{
                        ...
                        "forwarding_protocol": {
                            ...
                            Default: string(frontdoor.HttpOnly),
                        },
                        "frontend_endpoints": {
                            Type: schema.TypeList,
                            Optional: true,
...
func resourceArmFrontDoorCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, frontDoorName)
        if err != nil {
        ...
        if existing.ID != nil && *existing.ID != "" {
            return tf.ImportAsExistsError("azurerm_front_door", *existing.ID)
        }
    }
...
func expandArmFrontDoorRoutingRule(input []interface{}) *[]frontdoor.RoutingRule {
    ...
    for _, item := range input {
        ...
        cacheConfiguration := v["cache_configuration"].([]interface{})
        backendPool := v["backend_pool"].([]interface{})
        ...
        result := frontdoor.RoutingRule{
            ...
            RoutingRuleProperties: &frontdoor.RoutingRuleProperties{
                ...
                BackendPool: expandArmFrontDoorSubResource(backendPool),
                CacheConfiguration: expandArmFrontDoorCacheConfiguration(cacheConfiguration),
...
func expandArmFrontDoorCacheConfiguration(input []interface{}) *frontdoor.CacheConfiguration {
    if len(input) == 0 {
        return nil
    }
    v := input[0].(map[string]interface{})

    queryParameterStripDirective := v["query_parameter_strip_directive"].(string)
    dynamicCompression := v["dynamic_compression"].(string)

    result := frontdoor.CacheConfiguration{
        DynamicCompression: frontdoor.DynamicCompressionEnabled(dynamicCompression),
        QueryParameterStripDirective: frontdoor.Query(queryParameterStripDirective),
    }
    return &result
}
...
func flattenArmFrontDoorRoutingRule(input *[]frontdoor.RoutingRule) []interface{} {
    ...
    for _, item := range *input {
        ...
        if routingRuleProperties := item.RoutingRuleProperties; routingRuleProperties != nil {
            ...
            v["backend_pool"] = flattenArmFrontDoorSubResource(routingRuleProperties.BackendPool)
            v["cache_configuration"] = flattenArmFrontDoorCacheConfiguration(routingRuleProperties.CacheConfiguration)
            if customForwardingPath := routingRuleProperties.CustomForwardingPath; customForwardingPath != nil {
                v["custom_forwarding_path"] = *customForwardingPath
            }
            v["enabled_state"] = string(routingRuleProperties.EnabledState)
            v["forwarding_protocol"] = string(routingRuleProperties.ForwardingProtocol)
...
func flattenArmFrontDoorCacheConfiguration(input *frontdoor.CacheConfiguration) []interface{} {
    if input == nil {
        return make([]interface{}, 0)
    }
    ...
    result["dynamic_compression"] = string(input.DynamicCompression)
    result["query_parameter_strip_directive"] = string(input.QueryParameterStripDirective)
