...
func testCheckAzureRMServiceTopologyExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMServiceTopologyDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).serviceTopologiesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_service_topology" {
            continue
        }
