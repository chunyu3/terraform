...
func testCheckAzureRMExportConfigurationExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMExportConfigurationDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).exportConfigurationsClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_export_configuration" {
            continue
        }
