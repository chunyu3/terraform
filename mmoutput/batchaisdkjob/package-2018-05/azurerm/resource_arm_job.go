        ...
        Schema: map[string]*schema.Schema{
            ...
            "scheduling_priority": {
                ...
                ValidateFunc: validation.StringInSlice([]string{
                    string(batchaisdk.low),
                    string(batchaisdk.normal),
                    string(batchaisdk.high),
                }, false),
                Default: string(batchaisdk.low),
            },
...
func resourceArmJobCreateUpdate(d *schema.ResourceData, meta interface{}) error {
    ...
    if features.ShouldResourcesBeImported() && d.IsNewResource() {
        existing, err := client.Get(ctx, resourceGroup, workspaceName, experimentName, jobName)
        if err != nil {
            if !utils.ResponseWasNotFound(existing.Response) {
                return fmt.Errorf("Error checking for present of existing Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", jobName, experimentName, workspaceName, resourceGroup, err)
            }
        }
    ...
    schedulingPriority := d.Get("scheduling_priority").(string)
    ...
    parameters := batchaisdk.JobCreateParameters{
        JobBaseProperties: &batchaisdk.JobBaseProperties{
            ...
            CustomMpiSettings: expandArmJobCustomMpiSettings(customMpiSettings),
            ...
            HorovodSettings: expandArmJobHorovodSettings(horovodSettings),
            ...
            SchedulingPriority: batchaisdk.JobPriority(schedulingPriority),
    ...
    future, err := client.Create(ctx, resourceGroup, workspaceName, experimentName, jobName, parameters)
    if err != nil {
        return fmt.Errorf("Error creating Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", jobName, experimentName, workspaceName, resourceGroup, err)
    }
    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        return fmt.Errorf("Error waiting for creation of Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", jobName, experimentName, workspaceName, resourceGroup, err)
    }


    resp, err := client.Get(ctx, resourceGroup, workspaceName, experimentName, jobName)
    if err != nil {
        return fmt.Errorf("Error retrieving Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", jobName, experimentName, workspaceName, resourceGroup, err)
    }
    if resp.ID == nil {
        return fmt.Errorf("Cannot read Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q) ID", jobName, experimentName, workspaceName, resourceGroup)
    }
    d.SetId(*resp.ID)
...
func resourceArmJobRead(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        ...
        return fmt.Errorf("Error reading Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", jobName, experimentName, workspaceName, resourceGroup, err)
    }
    ...
    if jobBaseProperties := resp.JobBaseProperties; jobBaseProperties != nil {
        if err := d.Set("caffe2settings", flattenArmJobCaffe2Settings(jobBaseProperties.Caffe2settings)); err != nil {
            return fmt.Errorf("Error setting `caffe2settings`: %+v", err)
        }
        ...
        if err := d.Set("custom_mpi_settings", flattenArmJobCustomMpiSettings(jobBaseProperties.CustomMpiSettings)); err != nil {
            return fmt.Errorf("Error setting `custom_mpi_settings`: %+v", err)
        }
        ...
        if err := d.Set("horovod_settings", flattenArmJobHorovodSettings(jobBaseProperties.HorovodSettings)); err != nil {
            return fmt.Errorf("Error setting `horovod_settings`: %+v", err)
        }
        ...
        d.Set("scheduling_priority", string(jobBaseProperties.SchedulingPriority))
...
func resourceArmJobDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    future, err := client.Delete(ctx, resourceGroup, workspaceName, experimentName, jobName)
    if err != nil {
        if response.WasNotFound(future.Response()) {
            return nil
        }
        return fmt.Errorf("Error deleting Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", jobName, experimentName, workspaceName, resourceGroup, err)
    }

    if err = future.WaitForCompletionRef(ctx, client.Client); err != nil {
        if !response.WasNotFound(future.Response()) {
            return fmt.Errorf("Error waiting for deleting Job (Job Name %q / Experiment Name %q / Workspace Name %q / Resource Group %q): %+v", jobName, experimentName, workspaceName, resourceGroup, err)
        }
    }
...
func expandArmJobContainerSettings(input []interface{}) *batchaisdk.ContainerSettings {
    ...
    shmSize := v["shm_size"].(string)

    result := batchaisdk.ContainerSettings{
        ImageSourceRegistry: expandArmJobImageSourceRegistry(imageSourceRegistry),
        ShmSize: utils.String(shmSize),
    }
    return &result
}

func expandArmJobCustomMpiSettings(input []interface{}) *batchaisdk.CustomMpiSettings {
    if len(input) == 0 {
        return nil
    }
    v := input[0].(map[string]interface{})
    ...
    result := batchaisdk.CustomMpiSettings{
...
func expandArmJobHorovodSettings(input []interface{}) *batchaisdk.HorovodSettings {
    if len(input) == 0 {
        return nil
    }
    v := input[0].(map[string]interface{})
    ...
    result := batchaisdk.HorovodSettings{
        CommandLineArgs: utils.String(commandLineArgs),
        ProcessCount: utils.Int32(int32(processCount)),
        PythonInterpreterPath: utils.String(pythonInterpreterPath),
        PythonScriptFilePath: utils.String(pythonScriptFilePath),
...
func flattenArmJobCaffe2Settings(input *batchaisdk.Caffe2Settings) []interface{} {
    if input == nil {
...
func flattenArmJobContainerSettings(input *batchaisdk.ContainerSettings) []interface{} {
    ...
    if shmSize := input.ShmSize; shmSize != nil {
        result["shm_size"] = *shmSize
    }

    return []interface{}{result}
}

func flattenArmJobCustomMpiSettings(input *batchaisdk.CustomMpiSettings) []interface{} {
    if input == nil {
        return make([]interface{}, 0)
    }
...
func flattenArmJobHorovodSettings(input *batchaisdk.HorovodSettings) []interface{} {
    if input == nil {
        return make([]interface{}, 0)
    }
