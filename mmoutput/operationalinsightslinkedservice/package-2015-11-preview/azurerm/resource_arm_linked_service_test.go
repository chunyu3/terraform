...
func testCheckAzureRMLinkedServiceExists(resourceName string) resource.TestCheckFunc {
...
func testCheckAzureRMLinkedServiceDestroy(s *terraform.State) error {
    client := testAccProvider.Meta().(*ArmClient).linkedServicesClient
    ctx := testAccProvider.Meta().(*ArmClient).StopContext

    for _, rs := range s.RootModule().Resources {
        if rs.Type != "azurerm_linked_service" {
            continue
        }
